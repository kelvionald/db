...
INSERT INTO book (isbn, name, page_num, publication_year)
VALUES ('978-3-..', 'Библия', 100, 10);
-- 	3. С чтением значения из другой таблицы
-- 	INSERT INTO table2 (column_name(s)) SELECT column_name(s) FROM table1;
INSERT INTO author (firstname, lastname, book_num, birthday)
SELECT firstname, lastname, read_num, birthday
FROM reader;
...
SELECT *
FROM reader
ORDER BY reader_id DESC, firstname DESC
LIMIT 1;
...
SELECT reader.reader_id, date_part('year', birthday)
FROM reader;
...
ORDER BY reader.reader_id ASC
LIMIT 1;
--     3. LEFT JOIN трех таблиц + WHERE по атрибуту из каждой таблицы
SELECT r.reader_id, i.issue_date, c.copy_id
FROM reader r
